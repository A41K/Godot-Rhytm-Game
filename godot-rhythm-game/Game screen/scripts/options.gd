...
func _open_options() -> void:
	...
	for child in get_parent().get_children():
		if child != self and child.name.begins_with("Options"):
			return 

	var options_scene = load("res://Game screen/scenes/options.tscn")
	if options_scene:
		var options_window = options_scene.instantiate()
		get_parent().add_child(options_window)
		
		options_window.position = Vector2(0, 0)
	else:
		print("Failed to load options scene")
