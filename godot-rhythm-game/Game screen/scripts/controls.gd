...
func _open_achievements() -> void:
		for child in get_parent().get_children():
				if child.name == "ControlsWindowInstance":
						return 

		var achpv_scene = load("res://Game screen/scenes/controls.tscn")
		if achpv_scene:
				var achpv_window = achpv_scene.instantiate()
				achpv_window.name = "ControlsWindowInstance"
				get_parent().add_child(achpv_window)
				achpv_window.position = Vector2(0, 0)
		else:
				print("Failed to load controls scene")
